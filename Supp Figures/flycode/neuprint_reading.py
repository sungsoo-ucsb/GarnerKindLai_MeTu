...
def plot_comparison(compare_types, datasets={"FAFB": "LR", "Hemibrain": "R"},
    ...
    compare_types : list-like
        The types that will be compared in the plot.
    datasets : dict
        The datasets used (keys) as well as their hemispheres (values) as
        a string ("LR", "L", or "R").
    plot_name : str, optional
        The name of the outputted plot. The default is "Neuron Counts".
    save_figure : bool, optional
    ...
    neuron_counts = collections.defaultdict(lambda: np.array([], dtype=int))
    dataset_hemispheres = []
    for i,j in itertools.product(compare_types,list(datasets.keys())):
        for k in datasets[j]:
            hemis = "Left" if k=="L" else "Right"
            neur_count = len(comparison_data[
                        (comparison_data["Generalized Type"]==i) &
                        (comparison_data["Dataset"]==j) &
                        (comparison_data["Hemisphere"]==hemis)])
            type_label = f"{j} {hemis}"
            dataset_hemispheres.append(type_label)
            neuron_counts[type_label] = np.append(neuron_counts[type_label], 
                                                   neur_count)
    for i in neuron_counts:
        neuron_counts[i] = np.expand_dims(neuron_counts[i], axis=0)
    
    all_colors = {"FAFB Left": "#1f77b4",
                  "FAFB Right": "#d62728",
                  "Hemibrain Left": "#ffa90e",
                  "Hemibrain Right": "#2ca02c",
                  "FIB-SEM Right": "#BC27D6"}
    colors = [all_colors[x] for x in neuron_counts]
